...
describe('createLetterCodes', () => {
    ...
    it('should throw an error if symbols.length > numberRange', () => {
        ...
        expect(() => createLetterCodes(symbols, numberRange)).toThrowError(
            'Number range smaller than symbols length',
        );
    });
...
describe('checkSecretMessage', () => {
    it('string of 15 symbols or less without spaces is not split', () => {
        ...
        expect(splitSecrectMessageToSubstrings(inputShort)).toEqual(['ABCD']);
        expect(splitSecrectMessageToSubstrings(input)).toEqual([
            'ABCDEFGHJKLMNOP',
        ]);
    });
    ...
    it('string of more than 15 symbols with a space on position less than 15 is split into two , one up to the space and another after it', () => {
        ...
        expect(splitSecrectMessageToSubstrings(inputShort)).toEqual([
            'ABCDEFGHJKLMNO',
            'PRQ',
        ]);
    });
    ...
    it('the string can be split only to 4  maximum, if the string exceeds 4 substring, the rest is cut', () => {
        const input =
            'ABCDEFGHJKLMNOPABCDEFGHJKLMNOPABCDEFGHJKLMNOPABCDEFGHJKLMNOP';
        const excessiveInput =
            'ABCDEFGHJKLMNOPABCDEFGHJKLMNOPABCDEFGHJKLMNOPABCDEFGHJKLMNOPAA';
        const excessiveInputWithSpaces =
            'ABCDEFGHJKLMNOP ABCDEFGHJKLMNOP ABCDEFGHJKLMNOP ABCDEFGHJKLMNOP';
        const excessiveInputWithSpacesAndMore =
            'ABCDEFGHJKLMNOP ABCDEFGHJKLMNOP ABCDEFGHJKLMNOP ABCDEFGHJKLMNOPAA';
        expect(splitSecrectMessageToSubstrings(input)).toEqual([
            'ABCDEFGHJKLMNOP',
            'ABCDEFGHJKLMNOP',
            'ABCDEFGHJKLMNOP',
        ...
        expect(splitSecrectMessageToSubstrings(excessiveInput)).toEqual([
            'ABCDEFGHJKLMNOP',
            'ABCDEFGHJKLMNOP',
            'ABCDEFGHJKLMNOP',
            'ABCDEFGHJKLMNOP',
        ...
        expect(
            splitSecrectMessageToSubstrings(excessiveInputWithSpaces),
        ).toEqual([
            'ABCDEFGHJKLMNOP',
            'ABCDEFGHJKLMNOP',
            'ABCDEFGHJKLMNOP',
        ...
        expect(
            splitSecrectMessageToSubstrings(excessiveInputWithSpacesAndMore),
        ).toEqual([
            'ABCDEFGHJKLMNOP',
            'ABCDEFGHJKLMNOP',
            'ABCDEFGHJKLMNOP',
