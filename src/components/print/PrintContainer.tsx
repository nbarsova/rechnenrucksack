...
import './Print.css';
import { puzzleKeys, puzzles } from '../../app/puzzles.tsx';
...
const PrintContainer = () => {
    ...
    switch (currentPuzzle) {
        case puzzleKeys.TREASURE_PUZZLE_KEY:
            puzzleComponent = solution ? (
                <PrintTreasureSolutionPage
                    equations={JSON.parse(
                        getFromStorage(EQUATIONS_PARAMETER_NAME) as string,
                    )}
                    currentTarget={JSON.parse(
                        getFromStorage(CURRENT_TARGET_PARAMETER_NAME) as string,
                    )}
                    directions={JSON.parse(
                        getFromStorage(DIRECTIONS_PARAMETER_NAME) as string,
                    )}
                />
            ) : (
                <PrintTreasurePage
                    numberRange={Number(
                        getFromStorage(NUMBER_RANGE_PARAMETER_NAME),
                    )}
                    directions={JSON.parse(
                        getFromStorage(DIRECTIONS_PARAMETER_NAME) as string,
                    )}
        ...
        case puzzleKeys.SECRET_CODE_PUZZLE_KEY:
            puzzleComponent = (
                <SecretCodePrintPage
                    ...
                    letterCodes={JSON.parse(
                        getFromStorage(LETTER_CODES_PARAMETER_NAME) as string,
                    )}
                    message={JSON.parse(
                        getFromStorage(SECRET_MESSAGE_PARAMETER_NAME) as string,
                    )}
                    parentHeight={canvasHeight}
        ...
        case puzzleKeys.MONSTER_PUZZLE_KEY:
            const monsterAmount = JSON.parse(
                getFromStorage(MONSTERS_AMOUNT_PARAMETER_NAME) as string,
            );
            const monsterEquations = JSON.parse(
                getFromStorage(EQUATIONS_PARAMETER_NAME) as string,
            );
            puzzleComponent = (
                <MonsterPrintPage
    ...
    return (
        <div className="printPreviewContainer" ref={printElementDiv}>
            <div className="printHeader">
                <div className="printTitle">{puzzleTitle}</div>
                <div className="nameDate">
                    <div className="nameDateLine">
                        <span className="equationText">
