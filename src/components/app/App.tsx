...
const App = () => {
    ...
    const currentPuzzle = location.pathname.slice(
        ROOT_PATH.length + 1,
        location.pathname.length,
    );

    const renderPuzzleInBar = (puzzle: any) => (
        <Link
            key={puzzle.key}
            to={puzzle.key}
            className={
    ...
    return (
        <IntlProvider locale={locale} messages={messages[locale]}>
            <Routes>
                ...
                >
                    <Route
                        path={ROOT_PATH + '/' + puzzleKeys.TREASURE_PUZZLE_KEY}
                        element={<TreasureHunt />}
                    />
                    <Route
                        path={
                            ROOT_PATH + '/' + puzzleKeys.SECRET_CODE_PUZZLE_KEY
                        }
                        element={<SecretCode />}
                    />
                    <Route
                        path={ROOT_PATH + '/' + puzzleKeys.MONSTER_PUZZLE_KEY}
                        element={<LockMonster />}
                    />
                </Route>
                <Route
