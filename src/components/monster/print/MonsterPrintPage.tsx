...
const MonsterPrintPage = (props: MonsterPrintPageProps) => {
    const { equations, monstersAmount, showAnswers, parentHeight } = props;

    const monsterCellHeight =
        monstersAmount === 2 ? parentHeight - 20 : parentHeight / 2 - 10;
    const monsterGridStyle =
        monstersAmount <= 4 ? 'repeat(2, 1fr)' : 'repeat(3, 1fr)';
    const monsterPics = [
        monster1,
        monster2,
        monster3,
    ...
    const monsterEquations = createMonsterEquationsArray(
        equations,
        monstersAmount,
    );
    ...
    return (
        ...
        >
            {monsterEquations &&
                monsterEquations.map(
                    (equationSet: Equation[], index: number) => {
                        const monsterIndex = normalRandom(
                            0,
                            monsterPics.length - 1,
                        );
